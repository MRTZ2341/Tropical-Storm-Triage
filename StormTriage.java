...
class EmergencyRoom {
    ...
    public Patient findWaitingPatientById(int id) {
    ...
    public Patient treatPatientById(int id) {
        Patient selected = findWaitingPatientById(id);
        if (selected == null) return null;
        waitingQueue.remove(selected);
        selected.markTreatmentStart();
        treatedLog.add(selected);
        return selected;
    }
...
public class StormTriage {
    ...
    /** Let the user manually choose any waiting patient to treat. */
    static void menuTreatAnyPatient(EmergencyRoom er) {
        section("Treat Any Patient in Hospital");
        if (er.isEmpty()) {
            System.out.println("  ⚠️  No patients are currently waiting.");
            return;
        }
        ...
        int patientId = promptInt("Enter patient ID to treat", 1001, Integer.MAX_VALUE);
        Patient selected = er.findWaitingPatientById(patientId);

        if (selected == null) {
            System.out.printf("  ⚠️  No waiting patient found with ID #%d.%n", patientId);
            return;
        }

        System.out.printf("  👁️  Selected : %s%n", selected.getName());
        System.out.printf("      Severity: %s%n", selected.getSeverity().label);
        System.out.printf("      Condition: %s%n", selected.getCondition());
        System.out.print("\n  Confirm treatment? (y/n): ");
        String confirm = sc.nextLine().trim().toLowerCase();

        if (confirm.equals("y") || confirm.equals("yes")) {
            Patient treated = er.treatPatientById(patientId);
            System.out.printf("%n  🏥 Treating: %-20s (%s)  ⏱ waited %s%n",
                    treated.getName(), treated.getSeverity().label, treated.formatWaitTime());
        } else {
            System.out.println("  ↩️  Treatment cancelled.");
    ...
    public static void main(String[] args) {
        ...
        while (true) {
            ...
            System.out.println("│  3) Treat any patient in hospital                        │");
            System.out.println("│  4) View waiting queue                                   │");
            System.out.println("│  5) View treatment log                                   │");
            System.out.println("│  6) Change priority mode                                 │");
            System.out.println("│  7) Exit                                                 │");
            System.out.println("└──────────────────────────────────────────────────────────┘");

            int choice = promptInt("Choose option", 1, 7);

            switch (choice) {
                case 1: menuAddPatient(er);      break;
                case 2: menuTreatNext(er);       break;
                case 3: menuTreatAnyPatient(er); break;
                case 4:
                ...
                case 5:
                    ...
                    break;
                case 6: menuChangePriority(er); break;
                case 7:
